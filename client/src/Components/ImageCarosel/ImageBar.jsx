...
//     useEffect(()=>{
//         Axios.post('/api', {query : props.restaurant , type: "data retrieve"})
...
class ImageBar extends Component{
    ...
    componentDidMount(){
        Axios.post('http://YumlyMicroservice-env-2.ynkqtbpzvp.us-east-2.elasticbeanstalk.com/api', {query : '1' , type: "data retrieve"})
    ...
    componentDidUpdate(prevProps){
        if (this.props.restaurant !== prevProps.restaurant) {
            Axios.post('http://YumlyMicroservice-env-2.ynkqtbpzvp.us-east-2.elasticbeanstalk.com/api', {query : this.props.restaurant , type: "data retrieve"})
               .then(restaurant=>this.setState({currentRestaurant: restaurant.data[0]}, this._triosCreate))
               .catch((err)=>{console.log(err, "ya borked it!")})
        }
    }
